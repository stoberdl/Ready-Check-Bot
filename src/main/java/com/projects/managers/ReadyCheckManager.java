...
public class ReadyCheckManager {
    ...
    public static void createReadyCheckResponse(Object event, String readyCheckId, List<Member> targetMembers, Member initiator, String description) {
        ...
        if (event instanceof SlashCommandInteractionEvent) {
            handleSlashCommandResponse((SlashCommandInteractionEvent) event, embed, mainButtons, saveButton, mentions, readyCheckId);
        } else if (event instanceof StringSelectInteractionEvent) {
            handleSelectMenuResponse((StringSelectInteractionEvent) event, embed, mainButtons, saveButton, mentions, readyCheckId);
        }
    }

    private static String buildCheckDescription(Member initiator, List<Member> targetMembers, String originalDescription) {
        ...
        if (originalDescription.contains("specific users")) {
            return "**" + initiator.getEffectiveName() + "** started a ready check for " + totalUsers + " users";
        }
    ...
    public static String findExistingReadyCheck(String guildId, SavedReadyCheck savedCheck, String initiatorId) {
    ...
    public static String findExistingReadyCheck(String guildId, List<String> targetUserIds, String initiatorId) {
    ...
    public static String createUserReadyCheck(String guildId, String channelId, String initiatorId, List<Member> targetMembers) {
        return createReadyCheck(guildId, channelId, initiatorId, null, targetMembers,
                "**Ready Check** for specific users");
    }

    public static String createReadyCheck(String guildId, String channelId, String initiatorId, String roleId, List<Member> targetMembers) {
        return createReadyCheck(guildId, channelId, initiatorId, roleId, targetMembers,
                "**Ready Check** for role members");
    }
    ...
    /**
     * Parse time input for "r in X" context - always treats numbers as minutes
     * Examples: "5", "30", "120"
     */
    private static long parseTimeInputAsMinutes(String timeInput) throws Exception {
        ...
        throw new IllegalArgumentException("For 'r in X', please use just the number of minutes (e.g., '5', '30')");
    }

    /**
     * Parse time input for "r at X" context - treats as time with smart AM/PM
     * Examples: "5", "7:30", "7pm", "19:30"
     */
    ...
    /**
     * Parse ambiguous formats with smart AM/PM detection
     * Handles: "7", "7:30" (no AM/PM specified)
     */
     ...
     */
    public static String scheduleReadyAt(String readyCheckId, String timeInput, String userId, JDA jda) {
        try {
            ...
            scheduler.schedule(() -> sendReadyReminder(readyCheckId, userId, jda), delayMinutes, TimeUnit.MINUTES);
     ...
     */
    public static String scheduleReadyAtSmart(String readyCheckId, String timeInput, String userId, JDA jda) throws Exception {
        ReadyCheck readyCheck = activeReadyChecks.get(readyCheckId);
        if (readyCheck == null) {
            throw new IllegalArgumentException("Ready check not found");
        }
        ...
        scheduler.schedule(() -> sendReadyReminder(readyCheckId, userId, jda), minutesUntilReady, TimeUnit.MINUTES);
     ...
     */
    public static String scheduleReadyUntil(String readyCheckId, String timeInput, String userId, JDA jda) {
        try {
            ...
            scheduler.schedule(() -> autoPassUser(readyCheckId, userId, jda), delayMinutes, TimeUnit.MINUTES);
    ...
    public static void updateReadyCheckEmbed(String readyCheckId, JDA jda) {
        ...
        if (wasCompleted && !nowCompleted && readyCheck.getCompletionMessageId() != null) {
            channel.retrieveMessageById(readyCheck.getCompletionMessageId()).queue(
                    message -> message.delete().queue(null, error -> {}),
                    error -> {}
            );
        ...
        channel.retrieveMessageById(readyCheck.getMessageId()).queue(message -> {
            ...
            message.editMessageEmbeds(embed.build())
                    .setComponents(
                            ActionRow.of(mainButtons),
                            ActionRow.of(saveButton)
                    ).queue();
        }, error -> {
    ...
    public static boolean markUserReady(String readyCheckId, String userId) {
        ReadyCheck readyCheck = activeReadyChecks.get(readyCheckId);
        if (readyCheck == null) {
      //      logger.warn("Attempted to mark user ready for non-existent ready check: {}", readyCheckId);
            return false;
        }
        ...
        boolean allReady = readyCheck.getTargetUsers().stream()
    ...
    public static void notifyAllReady(String readyCheckId, JDA jda) {
        ...
        if (getMentionPreference(readyCheckId)) {
            readyUserMentions = allUsers.stream()
        ...
        EmbedBuilder completedEmbed = new EmbedBuilder()
        ...
        channel.sendMessage(messageContent)
                .setEmbeds(completedEmbed.build()).queue(message -> {
    ...
    /**
     * Updated findActiveReadyCheckForUser to include passed users
     * This allows "r" commands to work even if user was previously passed
     */
    ...
    /**
     * Updated saveReadyCheck to prioritize most recently saved configs
     * Removes duplicates and adds the new save to the end (most recent)
     */
    public static void saveReadyCheck(String readyCheckId) {
        ...
        String newKey = guildId + "_" + (newSavedCheck.isUserBased() ?
                newSavedCheck.getUserIds().hashCode() : newSavedCheck.getRoleId());

        savedReadyChecks.remove(newKey);

        // remove any other entries for this guild that match the same configuration
        savedReadyChecks.entrySet().removeIf(entry -> {
     ...
     */
    public static void sendReadyCheckToChannel(TextChannel channel, String readyCheckId, List<Member> targetMembers, Member initiator, String description, JDA jda) {
        ReadyCheck readyCheck = activeReadyChecks.get(readyCheckId);
        if (readyCheck == null) return;
...
//        return savedReadyChecks.entrySet().stream()
//                .filter(entry -> entry.getKey().startsWith(guildId + "_"))
//                .map(Map.Entry::getValue)
//                .collect(Collectors.toList());
//    }
public static List<SavedReadyCheck> getSavedReadyChecks(String guildId) {
    List<SavedReadyCheck> checks = savedReadyChecks.entrySet().stream()
            .filter(entry -> entry.getKey().startsWith(guildId + "_"))
            .map(Map.Entry::getValue)
            .collect(Collectors.toList());
    ...
    private static String createReadyCheck(String guildId, String channelId, String initiatorId, String roleId, List<Member> targetMembers, String description) {
        String readyCheckId = UUID.randomUUID().toString();

        List<String> targetUserIds = targetMembers.stream()
                .map(Member::getId)
                .collect(Collectors.toList());

        ReadyCheck readyCheck = new ReadyCheck(readyCheckId, guildId, channelId, initiatorId, roleId, targetUserIds);
    ...
    private static void scheduleAutoUnready(String readyCheckId, String userId, int minutes, JDA jda) {
        ReadyCheck readyCheck = activeReadyChecks.get(readyCheckId);
        if (readyCheck != null) {
    ...
    private static void autoUnreadyUser(String readyCheckId, String userId, JDA jda) {
        ...
        String messageText = getMentionPreference(readyCheckId)
        ...
        channel.sendMessage(messageText)
                .queue(message -> scheduleMessageDeletion(message, 5));
    }
    ...
    private static void sendReadyReminder(String readyCheckId, String userId, JDA jda) {
        ...
        channel.sendMessage(messageText)
                .queue(message -> scheduleMessageDeletion(message, 3));
    }
    ...
    // UI Building Methods
    private static EmbedBuilder buildReadyCheckEmbed(ReadyCheck readyCheck, JDA jda, String description) {
        String status;
        Color color;
    ...
    private static String buildMemberStatus(ReadyCheck readyCheck, String userId, JDA jda) {
        ...
        if (readyCheck.getPassedUsers().contains(userId)) {
            ...
            if (readyCheck.getUserUntilTimes().containsKey(userId)) {
                ...
                return "✅ " + displayName + " (auto-unready in " + readyCheck.getUserTimers().get(userId) + "min)";
            } else {
                return "✅ " + displayName;
            }
    ...
    private static List<Button> createSaveButton(String readyCheckId) {
        return Collections.singletonList(
                Button.secondary("save_ready_" + readyCheckId, "💾")
        );
    }
    ...
    private static void handleSlashCommandResponse(SlashCommandInteractionEvent event, EmbedBuilder embed, List<Button> mainButtons, List<Button> saveButton, String mentions, String readyCheckId) {
    ...
    private static void handleSelectMenuResponse(StringSelectInteractionEvent event, EmbedBuilder embed, List<Button> mainButtons, List<Button> saveButton, String mentions, String readyCheckId) {
        event.reply(mentions)
                .setEmbeds(embed.build())
                .setComponents(
                        ActionRow.of(mainButtons),
    ...
    private static String formatTimeForDisplay(String originalInput, long delayMinutes, LocalTime readyTime) {
    ...
    public static class ReadyCheck {
        ...
        public ReadyCheck(String id, String guildId, String channelId, String initiatorId, String roleId, List<String> targetUserIds) {
            this.id = id;
        ...
        public Set<String> getReadyUsers() { return readyUsers; }
        public Map<String, Long> getScheduledUsers() { return scheduledUsers; }
        public Map<String, Integer> getUserTimers() { return userTimers; }
        public Map<String, String> getUserUntilTimes() { return userUntilTimes; }
        public Set<String> getPassedUsers() { return passedUsers; }
        public String getMessageId() { return messageId; }
        public String getCompletionMessageId() { return completionMessageId; }
        public ReadyCheckStatus getStatus() { return status; }
        public long getLastInteractionTime() { return lastInteractionTime; }
        public long getCreatedTime() { return createdTime; }
        public String getDescription() { return description; }

        public void setMessageId(String messageId) { this.messageId = messageId; }
        public void setCompletionMessageId(String completionMessageId) { this.completionMessageId = completionMessageId; }
        public void setStatus(ReadyCheckStatus status) { this.status = status; }
        public void setLastInteractionTime(long lastInteractionTime) { this.lastInteractionTime = lastInteractionTime; }
    ...
    public static class SavedReadyCheck {
        ...
        public boolean isUserBased() { return userBased; }
        public boolean getMentionPeople() { return mentionPeople; }
        public void setMentionPeople(boolean mentionPeople) { this.mentionPeople = mentionPeople; }
    }
}