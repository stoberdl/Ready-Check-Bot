...
public class ReadyCommand implements Command {
    ...
    private void handleMixedTargetsReadyCheck(SlashCommandInteractionEvent event, String targetsInput, Member initiator, TextChannel channel, String guildId, boolean mentionPeople) {
        ParsedTargets parsed = parseTargetsInput(event, targetsInput);

        if (parsed.getAllMembers().isEmpty()) {
            event.reply("❌ No valid targets found! Examples:\n" +
                    "• `@GameRole` - Ready check for a role\n" +
                    "• `@Alice @Bob @Charlie` - Ready check for specific users\n" +
                    "• `@GameRole @Alice @Bob` - Ready check for role + additional users").setEphemeral(true).queue();
            return;
        }

        // Remove initiator from targets
        List<Member> targetMembers = parsed.getAllMembers().stream()
        ...
        String existingCheckId = ReadyCheckManager.findExistingReadyCheck(guildId, targetUserIds, initiator.getId());
        ...
        ReadyCheckManager.createReadyCheckResponse(event, readyCheckId, targetMembers, initiator, description);
    }
    ...
    private String buildDescription(Member initiator, ParsedTargets parsed) {
        StringBuilder desc = new StringBuilder("**" + initiator.getEffectiveName() + "** started a ready check");

        List<String> parts = new ArrayList<>();

        if (!parsed.getRoles().isEmpty()) {
            String roleNames = parsed.getRoles().stream()
    ...
    private static class ParsedTargets {
        ...
        public Set<Member> getDirectUsers() { return directUsers; }
        public List<Member> getAllMembers() { return new ArrayList<>(allMembers); }
    }

    private void handleSavedReadyCheck(SlashCommandInteractionEvent event, String guildId, Member initiator, TextChannel channel, boolean mentionPeople) {
        List<ReadyCheckManager.SavedReadyCheck> savedChecks = ReadyCheckManager.getSavedReadyChecks(guildId);

        if (savedChecks.isEmpty()) {
            event.reply("No saved ready check configurations found!\n" +
                    "**Usage:**\n" +
                    "• `/ready targets:@RoleName` - Ready check for a role\n" +
                    "• `/ready targets:@user1 @user2` - Ready check for specific users\n" +
                    "• Use 'Save for Later' button to save configurations").setEphemeral(true).queue();
            return;
        }
        ...
        if (savedChecks.size() == 1) {
            ...
            StringSelectMenu.Builder menuBuilder = StringSelectMenu.create("select_saved_ready_" + mentionPeople)
            ...
            for (ReadyCheckManager.SavedReadyCheck savedCheck : savedChecks) {
                if (savedCheck.isUserBased()) {
                    ...
                    menuBuilder.addOption("User Group (" + savedCheck.getUserIds().size() + " users)",
            ...
            event.reply("Select a saved ready check configuration:")
    ...
    private void startReadyCheckFromSaved(SlashCommandInteractionEvent event, ReadyCheckManager.SavedReadyCheck savedCheck, Member initiator, TextChannel channel, boolean mentionPeople) {
        if (savedCheck.isUserBased()) {
            ...
            // Get current members with the role (excluding initiator for now, we'll add them back in the manager)
            ...
            // Create ready check ID (this will include the initiator in the target users AND mark them as ready)
    ...
    private void startUserBasedReadyCheck(SlashCommandInteractionEvent event, String readyCheckId, List<Member> targetMembers, Member initiator) {
    ...
    private void startRoleBasedReadyCheck(SlashCommandInteractionEvent event, String readyCheckId, Role targetRole, List<Member> targetMembers, Member initiator) {
        ReadyCheckManager.createReadyCheckResponse(event, readyCheckId, targetMembers, initiator,
